...
To download Census data using the tidycensus package you need to get an API key.  You can get a Census API key [here]( https://api.census.gov/data/key_signup.html). Please fill in your name and Organization then submit to get your key. Once you have your key you need to put it somewhere that you can remember it. Note your key is like a password so you should make sure to secure it. I recommend that you save this API key in a `.Renviron` file in your project working directory. If you do this then you do not need to remember it. You also do not have to type it into your *.Rmd file.
When you clone my repo you will see file called `.RenvironChange`. You will need to edit this file in a text editor. The text editor in Windows is Notepad and on your Mac it is TextEdit. There is a line of code 

    census_api_key = `XXXX`

Please replace the XXXX with your key and maintain the single quotes please. Now change your filename to `.Renviron` . Save this in the same working directory. You are now ready to pull data from the Census Bureau. 

```{r set_up_API}
#readRenviron("C:/Users/ecsusan/Dropbox/2021VT-DSPG-FILES/2023SummerProgram/.Renviron") #read in your Census API key
readRenviron("~/.Renviron") #read in your Census API key
#Sys.getenv("census_api_key") #confirm you have read in your Census API key
```
In the code chunk above you will see the command `readRenviron("~/.Renviron")`. This command tells R to read the SSH key you shared in the .Renviron file on your hard drive. If all the files are in your working directory then you can use the shortened form ~/ to reference the .Renviron file. I did both and commented out line 64 in lieu of the shorter command on line 65.

In the code chunk above you will see the command  `Sys.getenv("census_api_key")`. This is a check to make sure your API key was read in properly. Note that I commented it out because once I checked I don't want you to see my key.
...
my_vars <- c(survey = "acs5", median.household.income ="B19013_001", 
             popululation = "B02001_001",
             median.gross.rent = "B25064_001")
